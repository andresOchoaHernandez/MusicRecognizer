...
%%Mediamente prendo circa i primi due minuti di ogni canzone secondi di canzone
range=[1, 32000*140];
...
load('songsDataset');
%%NOTES:
% -Prendere i primi frammenti delle canzoni è problematico e può portare a
%  risultati sbagliati, questo è probabilmente dovuto al fatto che la 
%  maggior parte delle canzoni inizia con momenti di 'silenzio'
%
% -In alcuni casi prendere frammenti 
%  piccoli(ad esempio L.blocco+L.blocco/10) può essere problematico
%%END NOTES

%[YT,FT]=audioread('..\recordings\test5-Song in dataset-lot of noise.mp3');
...
%Normalize maximum values between 0 and 1
...
for i=1:20
    out1=strcat('song#',int2str(i));
    out2=strcat(out1,' matching at -');
    out3=strcat(int2str(matchingPercentual(i)),'%');
    out=strcat(out2,out3);
    disp(out);
end
